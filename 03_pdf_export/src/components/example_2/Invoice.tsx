...
const Invoice = (): JSX.Element => {
  ...
  const InvoiceTitle = (): JSX.Element => (
  ...
  const Address = (): JSX.Element => (
    <View style={styles.titleContainer}>
      <View style={styles.spaceBetween}>
        ...
        <View>
          <Text style={styles.addressTitle}>7, Anemona Odede, </Text>
    ...
    <View style={styles.titleContainer}>
      <View style={styles.spaceBetween}>
        <View style={{ maxWidth: 200 }}>
          ...
          <Text style={styles.address}>{receipt_data.address}</Text>
        </View>
  ...
  const TableBody = (): JSX.Element[] =>
    receipt_data.items.map((receipt) => (
  ...
  const TableTotal = (): JSX.Element => (
    <View style={{ width: "100%", flexDirection: "row" }}>
      ...
      <View style={styles.tbody}>
        <Text>{receipt_data.items.reduce((sum, item) => sum + item.price * item.qty, 0)}</Text>
      </View>
    </View>
  );
